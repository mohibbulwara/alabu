...
export default function SellerProfilePage() {
    ...
    const fetchSellerData = async () => {
      ...
      try {
        ...
        if (fetchedSeller && fetchedSeller.role === 'seller') {
          ...
          const dishes = await getDishes({ sellerId: fetchedSeller.id });
  ...
  return (
    <div className="container mx-auto py-12">
      ...
      <h2 className="font-headline text-3xl font-bold mb-8 text-center md:text-left">
        All Dishes from {seller.shopName}
      </h2>
      ...
      {sellerDishes.length > 0 ? (
        ...
        <div className="text-center py-20">
          <p className="text-lg text-muted-foreground">{seller.name} has not added any dishes yet.</p>
        </div>
      )}
    </div>
  );
